...
apiClient.interceptors.response.use(
  ...
  (error: AxiosError) => {
    // Don't log expected 403 errors for endpoints not available to certain roles
    const isExpectedClientError = error.config?.url === '/api/dashboard/clients' && error.response?.status === 403;
    const isExpectedTeamError = error.config?.url === '/api/dashboard/teams/top-5' && error.response?.status === 403;
    const isExpectedError = isExpectedClientError || isExpectedTeamError;
...
export const teamAPI = {
  ...
  /**
   * Get team history by period
   */
  getHistory: async (id: number, period: string = 'WEEK'): Promise<TeamStatsDTO> => {
    const response = await apiClient.get<TeamStatsDTO>(`/api/dashboard/teams/${id}/history?period=${period}`);
    return response.data;
  },
};
// ============================================================================
...
updateCostPerHour: async (id: number | string, cost: number): Promise<string> => {
  ...
  /**
   * Get agent history by period (DAY, WEEK, MONTH, YEAR)
   */
  getHistory: async (period: string = 'WEEK'): Promise<AgentStatsDTO> => {
    const response = await apiClient.get<AgentStatsDTO>(`/api/dashboard/agent/history?period=${period}`);
    return response.data;
  },
};
...
export const clientAPI = {
  ...
  /**
   * Get clients for a specific team (filtered by team name)
   */
  getClientsByTeam: async (teamName: string): Promise<Client[]> => {
    const response = await apiClient.get<Client[]>(`/api/clients`);
    // Filter clients by team name from agent's mostCurrentUserGroup
    return response.data
      .filter((client: any) => client.agent?.mostCurrentUserGroup === teamName)
      .map((client: any) => ({
        ...client,
        date_creation: formatDateToYYYYMMDD(
          client.date_creation || 
...
export interface NovaStatsDTO {
  ...
  grandTotalBooked?: number;
  grandTotalTalkTimeHours?: number;
  grandTotalWaitTimeHours?: number;
...
import { Formation, FormationDto, FormationSession, FormationSessionDTO, FormationType, FormationStatus, SessionStatus, AgentHistory } from '../../types';
...
export const formationsAPI = {
  ...
  /**
   * Create a new formation (requires NOVA role via backend authorization)
   */
  ...
  /**
   * Update an existing formation (requires NOVA role via backend authorization)
   */
  updateFormation: async (id: string, formation: Formation): Promise<Formation> => {
    console.log('Updating formation:', id, formation);
    const response = await apiClient.put<Formation>(`/api/formations/${id}`, formation);
    return response.data;
  },

  /**
   * Delete a formation (requires NOVA role via backend authorization)
   */
  ...
  enrollFormation: async (formationId: string): Promise<FormationSession> => {
    ...
    console.log('[ENROLL DEBUG]', {
      ...
      hasToken: !!token,
      userRole,
      endpoint: `/api/formations/sessions/${formationId}/enroll`
    });
    ...
    try {
      ...
      // Log special case for NOVA users
      if (userRole === 'NOVA' && error.response?.status === 403) {
        console.warn('[ENROLL WARNING] NOVA users cannot enroll in formations - only create/manage them. Switch to AGENT or TEAM role to enroll.');
      }
  ...
  completeChapter: async (sessionId: string, chapterId: string): Promise<FormationSession> => {
    console.log('[COMPLETE CHAPTER]', { sessionId, chapterId });
    try {
      const response = await apiClient.post<FormationSession>(
        `/api/formations/sessions/${sessionId}/complete-chapter/${chapterId}`,
        {}
      );
      console.log('[COMPLETE CHAPTER SUCCESS]', response.data);
      return response.data;
    } catch (error: any) {
      console.error('[COMPLETE CHAPTER ERROR]', {
        status: error.response?.status,
        data: error.response?.data,
        message: error.message
      });
